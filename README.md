...
- 统一返回 `Result` / `Result<T>`，经 `ToHttpResult()` 映射 HTTP 状态码（如 201/400/404/204）

## API

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/tasks` | 创建（201 / 400） |
| GET | `/tasks?page=&pageSize=` | 分页列表 |
| POST | `/tasks/{id}/complete` | 标记完成（200 / 404） |
| DELETE | `/tasks/{id}` | 删除（204 / 404） |

## 持久化

默认使用 SQLite **内存库**（`ConnectionStrings:Default`）。改为文件库后启动时会自动 `Migrate`：

```json
"ConnectionStrings": {
  "Default": "Data Source=veloapp.db"
}
```
...
详见 `src/VeloApp.Api/Migrations/README.md`。
...
│   ├── Shared/          # AppDbContext、Result、ToHttpResult、PagedList
│   ├── Migrations/
│   └── Program.cs
